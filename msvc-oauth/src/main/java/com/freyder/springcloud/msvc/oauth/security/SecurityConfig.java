package com.freyder.springcloud.msvc.oauth.security;
...
return http.build();
        ...
        //Agregamos informacion adicional al token:
        @Bean
        OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
          return context -> {
                if (context.getTokenType().getValue() == OAuth2TokenType.ACCESS_TOKEN.getValue()) {
                    Authentication principal = context.getPrincipal();
                      context.getClaims()//los claims es pra agregar data adicional al token
                        .claim("data", "data adicional en el token")
                        .claim("roles", principal.getAuthorities()
                                .stream()
                                .map(GrantedAuthority::getAuthority)//se toman los roles de SpringSecurity
